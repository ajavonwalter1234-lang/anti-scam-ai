...
#### Option C: Google Device Code Authentication (Recommended for Sandbox)
If you cannot bypass the firewall, use the **Device Flow**. This generates a short code that you enter on a separate device (like your phone) or a new browser tab, bypassing all local network and iframe restrictions.

1. Run the application or script.
2. If standard authentication fails, the app will automatically prompt:
   `Go to: https://www.google.com/device`
   `Enter code: XXXX-XXXX`
3. Enter the code on your phone or a non-restricted browser.
4. The application will automatically detect the authorization and sync your Gmail data.

To force this mode in code:
```python
gmail_service.authenticate(use_device_flow=True)
```
